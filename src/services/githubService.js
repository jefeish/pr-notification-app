...
export class GitHubService {
  ...
  /**
   * Check if a pull request is ready to merge using GitHub's mergeable_state
   * @param {Object} context - GitHub context
   * @param {number} prNumber - Pull request number
   * @returns {Promise<Object>} Result with readiness status and mergeable_state
   */
  async checkPRReadyToMerge(context, prNumber) {
    Logger.debug(`Checking PR #${prNumber} mergeable state`);
    
    try {
      // Get PR information with mergeable_state
      const { data: pr } = await context.octokit.pulls.get({
        owner: context.payload.repository.owner.login,
        repo: context.payload.repository.name,
      ...
      // Readiness check - include any states that may require notification
      // Available States: clean, unstable, dirty, blocked, unknown, behind, divergent, has_hooks
      const notificationStates = ['clean', 'unstable'];
      const ready = notificationStates.includes(mergeableState);
      
      Logger.info(`PR #${prNumber} mergeable_state: ${mergeableState}, ready: ${ready}`);
      ...
      Logger.error(`Error checking PR #${prNumber} mergeable state`, error);
      return {
        success: false,
        error: error.message,
        ready: false,
